...
esp_err_t LittleFlash::init(const little_flash_config_t *config) {
  ...
  part = esp_partition_find_first(ESP_PARTITION_TYPE_DATA,
                                  ESP_PARTITION_SUBTYPE_ANY, cfg.part_label);
  if (part == NULL) {
...
ssize_t LittleFlash::write_p(void *ctx, int fd, const void *data, size_t size) {
  ...
  lfs_ssize_t written =
      lfs_file_write(&that->lfs, that->fds[fd].file, data, size);
...
off_t LittleFlash::lseek_p(void *ctx, int fd, off_t size, int mode) {
  ...
  lfs_soff_t pos =
      lfs_file_seek(&that->lfs, that->fds[fd].file, size, lfs_mode);
...
int LittleFlash::readdir_r_p(void *ctx, DIR *pdir, struct dirent *entry,
...
int LittleFlash::internal_read(const struct lfs_config *c, lfs_block_t block,
                               lfs_off_t off, void *buffer, lfs_size_t size) {
  ESP_LOGD(TAG, "%s - block=0x%08x off=0x%08x size=%d", __func__, block, off,
           size);

  LittleFlash *that = (LittleFlash *)c->context;

  esp_err_t err = esp_partition_read(
      that->part, (block * that->sector_sz) + off, buffer, size);

  return err == ESP_OK ? LFS_ERR_OK : LFS_ERR_IO;
}

int LittleFlash::internal_prog(const struct lfs_config *c, lfs_block_t block,
...
int LittleFlash::internal_erase(const struct lfs_config *c, lfs_block_t block) {
  ...
  esp_err_t err = esp_partition_erase_range(that->part, block * that->sector_sz,
                                            that->sector_sz);
